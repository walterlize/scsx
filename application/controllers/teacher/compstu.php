...
class Compstu extends CI_Controller {
    ...
    //选择或新增基地
    public function compstuList(){

    	$this->timeOut();
    	
    	$tea_num = $this->session->userdata('u_num');
    	//根据课程号查找审核成功的基地
    	$array=array('comp_audit_num'=>$tea_num);
    	
    	$this->load->model('m_company');

    	$offset = $this->uri->segment(4);
    	$company = $this->getCompanys($array,$offset);//全部基地
    	$num = $this->m_company->getstuNum($array);
    	
    	$config['base_url'] = base_url() . 'index.php/teacher/compstu/compstuList';
    	...
    	$this->load->view('teacher/compstu/compstu', $data);
    	$this->load->view('common/footer');
    }
    
    // 实验任务详细信息页面
    public function compstuDetail() {
    	//基地号
    	$show="display:none";
    	...
    	$data['show']=$show;
    	$this->load->view('common/header3');
    	$this->load->view('teacher/compstu/compstuDetail', $data);
    	$this->load->view('common/footer');
    }
    /*
    function companyNew(){
    	...
    	$this->load->view('common/header3');
    	$this->load->view('teacher/compstu/compstuEdit', $data);
    	$this->load->view('common/footer');
    }
    ...
    */
    ...
    function companyDelete(){
    	...
    	redirect('teacher/compmanage/compstuList');
    ...
    public function getCompanys($array,$offset) {
    	$this->timeOut();
    	$this->load->model('m_company');
    	$result = $this->m_company->getCompstu($array,PER_PAGE, $offset);
