...
class User extends CI_Controller {
    ...
    public function teaList() {
    	...
    	$num = $this->m_nteacher->getNum(array());
    	$offset = $this->uri->segment(4);
    ...
    public function getTeas($array,$offset) {
    	...
    	$result = $this->m_nteacher->getTeas($array, PER_PAGE, $offset);
    ...
    public function userListJD() {
    	$this->timeOut();
    
    	$this->load->model('m_user');
    	$array = array('roleId'=>4);
    	$num = $this->m_user->getNum1($array);
    	$offset = $this->uri->segment(4);
    ...
    public function sava_data($array, $connect) {
        ...
        foreach ($array as $tmp) {
            $this->load->model('m_user');
            $result = $this->m_user->insertUser($tmp,$collegeId);
            if ($result) {
                    $count++;    
            }
            $total++;
...
// 分页获取全部用户信息
    public function getUsers($array,$offset) {
        $this->timeOut();
        $this->load->model('m_user');
        $data = array();
        
        $result = $this->m_user->getUsers($array, PER_PAGE, $offset);
